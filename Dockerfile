...
RUN \
  ...
  apt-get update && \ 
  apt-get install -y git gconf2 gconf-service libnotify4 gvfs-bin xdg-utils libxss1 terminator firefox wget zsh xfce4 xubuntu-icon-themel && \
  apt-get clean && \
...
RUN mkdir -p /nobody/.config/terminator/

RUN echo "nobody ALL=(ALL) NOPASSWD: ALL" > /etc/sudoers.d/nobody && \
  chmod 0440 /etc/sudoers.d/nobody && \
  chown ${uid}:${gid} -R /nobody

COPY files/config /nobody/.config/terminator/config
